...
与 PPO 实现的区别:
    GRPO 不需要独立的 Critic (Value Model)，因为优势函数通过
    组内相对比较计算，不需要价值估计。因此本文件只包含策略模型。
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        # 预计算因果掩码 (下三角矩阵)
        # register_buffer 确保掩码随模型一起移动到目标设备
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 因果掩码: causal_mask 已通过 register_buffer 绑定到正确设备
...
class GPTLanguageModel(nn.Module):
    """
    完整的 GPT 语言模型，作为 GRPO 中的策略模型。

    在 GRPO 中的角色:
        - 策略模型 π_θ: 根据 prompt 生成一组 (Group) 回复
        - 输出: 词表上的概率分布 → 用于采样 token
        - GRPO 优化目标: 最大化组内相对优势的同时不偏离参考策略太远
    """
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        """
        B, T = input_ids.shape
        # 关键: 使用 input_ids.device 确保位置索引在正确的设备上
